...
(def host (nth *command-line-args* 2 nil))
(def password (nth *command-line-args* 3 nil))

(assert (and username host password)
        "No username, hostname/ip and password specified. Pass in username as first argument, host as second argument, and password as third")
        ...
        (for [sh shells]
          (do
            ...
            (ssh user-conf
                 ...
                 (assert (not (failed? (sudo-user {:username "root" :password password} (apt :install "bash")))))

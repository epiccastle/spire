...
circle-setup: ssh_test_key_rsa
	-lsb_release -a
	sudo /usr/sbin/sshd -f test/config/sshd_config -ddd -D & echo "$$!" > sshd.pid
	eval `ssh-agent` && ssh-add ssh_test_key_rsa && export && lein trampoline test; EXIT=$$?; sudo kill `cat sshd.pid`; exit $$EXIT
